...
**Streak length of 1 includes 1 hit, and 1 miss. Streak Length of 0 is just 1 miss**
...
**His typical streak length was 1. His longest streak was 4.**
...
**Similar distrubtion to Kobe's. Typical streak length is 1. Longest streak was 5.**
...
**Somewhat similar if you changed the seed. The exact sequence of shots would change, but the overall percentages would stay similar to the 45% we set**
...
**Kobe Bryant's distribution of streak length is similar to the simulated shooter, who does not have a hot hand.  This would suggest Kobe's shots are independent of his next one. Although, it's interesting you would expect a similar drop (to the Sim's shots) from Kobe's 2 streak to 3 streak that isnt there.**

```{r plot-streak-sim}
sim <- ggplot(data = sim_streak, aes(x = length)) +
  ggtitle("Sim") +
  geom_bar()
```

```{r plot-streak-kobe}
kobe <- ggplot(data = kobe_streak, aes(x = length)) +
  ggtitle("Kobe") +
  geom_bar()
```
