...
```{r}
yrbss_text_while_driving_30d <- yrbss["text_while_driving_30d"]
```

```{r}
yrbss_text_while_driving_30d$text_while_driving_30d = factor(yrbss_text_while_driving_30d$text_while_driving_30d, levels = c("NA", "did not drive", "0","1-2","3-5","6-9","10-19","20-29","30"), ordered = TRUE)
ggplot(yrbss_text_while_driving_30d, aes(x = text_while_driving_30d)) +
  geom_bar() +
...
```{r}
ggplot(no_helmet, aes(x = helmet_12m, fill = text_ind)) +
  geom_bar(position ="fill") +
geom_text(
    aes(label=signif(100*..count.. / tapply(..count.., ..x.., sum)[as.character(..x..)], digits=3) ),
    stat="count",
...
**Insert your answer here**
There little to none margin of error at Population Proportion 0 or 100%. As you approach 50% from either end, the margin of error increases to the maximum of ~0.03. 
...
**Insert your answer here**
**center is closer to 0, with wider spread, and normal shape**
...
**Insert your answer here**
**as you increase p, the center increases as well, the shape and spread dont seem to change much at all?**

8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

**Insert your answer here**
**the spread becomes smaller as n increases**
...
I followed the infer documentation you posted on slack, im still lost at if we should reject or not. I'm just going to submit, and hopefully i will understand. If there are resubmissions allowed, i may resubmit at later date if i get a better understanding.


Null: sleep and strength training are independent. those who sleep 10+ hours per day are NOT more
    likely to strength train every day of the week
Alternative: sleep and strength training are dependent. those who sleep 10+ hours per day are more
    likely to strength train every day of the week
...
yrbss2 <- yrbss %>%
  mutate(sleep_10 = ifelse(school_night_hours_sleep == "10+", "yes", "no")) %>%
    mutate(strength_train_every_day = ifelse(strength_training_7d == "7", "yes", "no"))

null <-  yrbss2 %>%
  drop_na(sleep_10) %>% # Drop missing values
  drop_na(strength_train_every_day) %>% # Drop missing values
  specify(sleep_10 ~ strength_train_every_day, success = "yes") %>%
...
visualize(null) +
  shade_p_value(obs_stat = observed, direction = "greater")


chisq_test(yrbss2, sleep_10 ~ strength_train_every_day)



```

specify(response = sleep_10,  explanatory = strength_training_7d , success = "yes") %>%
...
**Insert your answer here**
    5% chance, a type 1 is a a false positive, that would be the chance you would falsely detect a change when there wasnt one
