...
**The relationship between pf_score and pf_expression_control looks linear. If I knew a country's `pf_expression_control` I would be comfortable using a linear model to predict the personal freedom score **
...
**The two variables look moderately positively linearly correlated. Unusual observations: Pakistan in 2009 had a very high expression control score, but a much lower pf_score than expect. Inversely, Norway in 2009 had a very high pf score but a much lower expression control than expected. **

```{r}
ggplot(hfi, aes(x=pf_score, y=pf_expression_control)) +
  geom_point() +
  geom_text(aes(label=countries), size=3)+
 geom_text(aes(label=year), size=3,  nudge_x = 0.35) 
...
**Smallest Sum of Squares:  1058.914**
...
Let's consider this output piece by piece. First, the formula used to describe the model is shown at the top. After the formula you find the five-number summary of the residuals. The "Coefficients" table shown next is key; its first column displays the linear model's y-intercept and the coefficient of `pf_expression_control`. With this table, we can write down the least squares regression line for the linear model:
 ...
 haty = 4.61707 + 0.49143 times pf_expression_control
...
haty = 5.153687  + 0.349862 times pf_expressional_control


**It seems the more political pressure on media content, the higher a countries human freedom score is predicted to be. For this model, 57.75% of the variability in total human freedom score is explained by political pressure on media content**
...
**They would predict a pf_score of 7.909651. There is no residual for this prediction, its perfectly on our regression line**
...
**The residuals seem evenly distributed around 0. There doesnt seem to be much of a curve implying that a linear model is a good fit for our data**
...
**ef_legal_military is Military interference in rule of law and politics.**

```{r}
ggplot(hfi, aes(x=pf_score, y=ef_legal_military)) +
  geom_point()
```

```{r}
m3 <- lm(pf_score ~ ef_legal_military, data = hfi)
```
...
haty = 5.12769   + 0.32315  times ef_legal_military
...
**Our pf_expression_control model had a R squared of 63.42%. This new one with ef_legal_military has an r squared of 0.4073. This suggests ef_legal_military does not seem to predict personal freedom better than expression control as it explains less of the variability in freedom.**
...
**It's uncomfortable to see that Military Interference in Rule of Law and Politics seems to have a linear relationship with personal freedom. I was surprised at how many countries were at the max at 10 of military interference while still having really high pf_scores. I suppose there needs to be some amount of enforcement for countries law and politics to be effective, but its disappointing to think about. There does seem to be alot more variability in the residuals (compared to our previous models), and paired with the lower r squared, makes me think there are some countries where the military interference is definitely not helping (or predicting) peoples personal freedoms**
