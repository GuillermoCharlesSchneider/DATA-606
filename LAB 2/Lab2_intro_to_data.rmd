...
**The first graph with binwidth 30 seems to be a good mix of a granular view, and bucketing. The second one with binwidth 15, the buckets are almost too small and too many for clear comparison, at that point i'd rather see it just plotted with bin width 1. Although this one is the only one does seem to show there are some with negative delays. The final graph is an simple as possible, i kind of like it if youre trying to just make a simple point about where the majority of delays lie. **
...
**The distribution appears normally distributed or slightly right skewed, with a few positive outliers **
...
**JFK has the larger variance and IQR, suggesting there is more spread in their arrival delays**
```{r}
sfo_feb_flights %>%
  group_by(origin) %>%
  summarise(median_ad = median(arr_delay), iqr_ad = IQR(arr_delay), var(arr_delay), n_flights = n())
```
...
**1. The lowest median departure delay would give you a better idea of how most flights go, but would not account for   exceptionally high or low delay outliers**
**2.The lowest mean departure delay would give you a better idea of how the average flight would go accounting for those      outlier delays.**
...
**It looks vaguely logarithmic. As distance increase, avg speed also increases but at a decreasing rate**
```{r}
...
** Its maximum 25-30 mins. If your arr delay and dep delay delay are inverse, you will be on time. anything under the -1 slope should get the person there on time? the greatest dep_delay thats still under that slope is around the 25 mark**
```{r}

q9 <- nycflights %>%
   filter(carrier == "AA" | carrier =="DL" | carrier =="UA")

ggplot(data = q9, aes(x = dep_delay, y = arr_delay, color=carrier)) +
  geom_point() +
 geom_abline(slope = -1)
