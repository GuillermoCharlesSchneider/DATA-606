...
**Identical, probably not. Similar, yes. Our random samples are pulling from the same true pop of 62:38 **
...
**95% of samples from our true population will fall between those bounds**
...
**Yes, our CI falls between our range of 0.5-0.75**
...
**95%, b/c of our 95% confidence interval we should expect about 5% of the intervals to not capture our true pop mean**
...
**44/50, or 88%, Not exactly equal to the confidence level.**
...
**I would expect a confidence interval at 70% to be narrower, as we're less confident that the range will contain the true pop in it.**
...
**70% of samples from our true population will fall between lower_ci 0.55 and upper_ci 0.683 **
```{r}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
...
**39/50 or 78%, include the true pop proportion. Its a little higher than the 70% i selected, but its similar.**
...
**confidence interval 99%. 99% of samples from our true population will fall between lower_ci 0.45 and upper_ci 0.783. 50/50 or 100% captured the true pop proportion, as expected**
...
**as samples sizes increase, the intervals width decreases, as the standard error decreases**
...
**Changing the number of bootstrap samples does not seem to effect the standard error. as the sample size doesnt change between bootstraps**
