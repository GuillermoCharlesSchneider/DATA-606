...
**74:26 from the sample is similar to the 80:20 population**
...
**Probably not, proportion will have a different random sample that will be close to the true pop 80:20 but probably a different proportion.  **
...
**84:16 is also similar to the true pop 80:20. As the sample size increases, the samples would provide a more accurate estimate of the population proportion.**
...
# Overlay with transparent density plot +
      geom_vline(aes(xintercept=mean(p_hat, na.rm=T)),   # Ignore NA values for mean
...
**4 elements, 15000 observations. It's normally distributed, with center around 0.2 which is expected with our 80:20 pop proportion**
...
**21  observations. I don't quite understand why there are less than 25 observations. Each observation represents a sample, and there are only 21 unique samples? 4 of those samples were exactly the same as one of the others? Why didn't this happen with the 15000 size, is just less likely with a larger sample size?**
```{r}
set.seed(111)
...
**Mean stays similar, the SE decrease, and the shape becomes more normal as the sample size increases. the numbers dont cha nge as we increase the number of simulations**

left skewed
Mean of sampling distribution = 0.22
SE of sampling distribution = 0.11

normal
Mean of sampling distribution = 0.2
SE of sampling distribution = 0.06

normal
Mean of sampling distribution = 0.2
SE of sampling distribution = 0.04
...
**11:4. I estimate 73% of the population think the work scientists do enhances their lives **
```{r}
set.seed(1234)
sampQ7 <- global_monitor %>%
  sample_n(15)
```
...
**I would guess the 80% of the population think the work scientists do enhances their live.**
...
**The shape looks much more normal than the 15 sample size.  Still would guess 80%.**
```{r}
set.seed(111)
...
**Sample 3 has a smaller spread. If I was concerned with making estimates that are more often close to the true value,I would prefer a sampling distribution with a small spread, **
